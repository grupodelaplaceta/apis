...
export function normalizarContrato(c) {
  return {
    ...
    eip: String(c.eip || "").trim().toUpperCase(),
...
export async function cerrarPeriodo(periodo, opciones = {}) {
  ...
  // El plazo es GLOBAL y mensual: todo contrato activo entra en el ciclo del
  // mes, sea cual sea la `frequency` heredada de la app (que por defecto es
  // Weekly). Si algún día se quieren ciclos semanales, se decide aquí.
  ...
  for (const contrato of contratos) {
    ...
    if (pagar && doc.status !== "Paid") {
      ...
      resultados.push({ contratoId: contrato.id, periodoId: doc.id, pagado: doc.status === "Paid", estado: doc.status, netoPz: doc.netoPz });
    }
  }
 ...
 * Es idempotente: lo ya pagado no se repite, y lo que falló (por ejemplo por
 * falta de saldo en la cuenta de la empresa) se reintenta en la siguiente pasada.
 */
export async function procesarVencimientos(hoy = new Date(), opciones = {}) {
  ...
  for (const periodo of [anterior, actual]) {
    ...
    if (r.contratos > 0) procesados.push({ periodo, pagar: r.pagar, contratos: r.contratos, pagados: r.resultados.filter((x) => x.pagado).length, resultados: r.resultados });
  }
...
export async function estadoNominas(filtro = {}, opciones = {}) {
  // Las lecturas web/app deben ser rápidas y no bloquearse con cierres que
  // pueden pagar muchos contratos. El cron y las llamadas administrativas
  // siguen usando el procesamiento perezoso por defecto.
  if (!opciones.skipAutoProcess) {

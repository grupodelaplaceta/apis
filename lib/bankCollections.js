...
function normalizeTreasuryIdentity(state) {
  ...
  return {
    ...
    transactions: Array.isArray(state.transactions) ? state.transactions.map((transaction) => ({
      ...transaction,
      fromAccountId: canonicalTglpId(transaction.fromAccountId),
      toAccountId: canonicalTglpId(transaction.toAccountId),
      IBAN_Origin: canonicalTglpId(transaction.fromAccountId) === TGLP_ID ? TGLP_OFFICIAL_IBAN : transaction.IBAN_Origin
    })) : state.transactions
  };
}
...
function applyServerTransaction(accounts, currentTransactions, currentOperations, paymentLinks, transaction, config = {}) {
  ...
  if (["PAYMENT_LINK", "PLACETA_SEND_LINK"].includes(transaction.concept)) {
    ...
    if (transaction.concept === "PAYMENT_LINK") {
      if (transaction.toAccountId !== link.creatorAccountId) return denied("payment_link_recipient_mismatch");
    } else {

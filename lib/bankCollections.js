...
export async function readBankState() {
  ...
  // Las colecciones no dependen entre sí: leerlas en paralelo reduce mucho la
  // latencia del backend y evita que una colección lenta bloquee todo el banco.
  const entries = await Promise.all(listSpecs.map(async ([stateKey, collectionName, idKey]) => [
    stateKey,
    dedupeBy(await db.collection(collectionName).find({}).project({ _id: 0 }).toArray(), idKey)
  ]));
  const state = Object.fromEntries(entries);
  const [treasuryConfig, meta] = await Promise.all([
    leerTreasuryConfigConNormativa(db),
    db.collection(bankCollections.meta).findOne({ _id: "state" })
  ]);
  state.treasuryConfig = treasuryConfig;
...
export async function writeBankState(state, options = {}) {
  ...
  try {
    ...
    await Promise.all(listSpecs.map(async ([stateKey, collectionName, idKey]) => {
...
function transactionNeedsProcessing(currentTransactions, transaction) {
  ...
  if (known.status === "Cancelled") return false;
  if (known.status !== "Settled" && transaction.status === "Settled") return true;
  // Revalidar Pending permite cancelar una operación que perdió su saldo
  // antes de liquidarse, en vez de conservar un estado engañoso.
  if (known.status === "Pending" && transaction.status === "Pending") return true;
  return false;
}

export function reconcileIncomingState(current, incoming, updatedAt) {
  // Incluso el primer estado debe pasar por la reconciliación autoritativa.
  // Saltarse este paso permitía persistir Settled/Pending sin comprobar saldos.
  ...
  const newTransactions = (incoming.transactions || [])
    ...
    .sort((left, right) => {
      // Una comisión/IVA dependiente siempre se procesa después de su
      // operación principal, aunque ambas tengan la misma fecha.
      if (left.originalTransactionId === right.id) return 1;
      if (right.originalTransactionId === left.id) return -1;
      return Date.parse(left.createdAt || updatedAt) - Date.parse(right.createdAt || updatedAt);
    });
  ...
  for (const transaction of newTransactions) {
    ...
    if (!result.ok) {
      ...
      // Un rechazo nunca debe dejar una operación aparentando estar en curso.
      // Se conserva el registro para trazabilidad, pero como estado terminal.
      const idx = nextTransactions.findIndex((item) => item.id === transaction.id);
      const cancelled = cancelledTransaction(currentTransactions.get(transaction.id) || transaction, transaction, result.reason, updatedAt);
      if (idx >= 0) nextTransactions.splice(idx, 1, cancelled);
      else nextTransactions.push(cancelled);
      currentTransactions.set(transaction.id, cancelled);
      continue;
    }
  ...
  // Limpieza de efectos secundarios huérfanos. Si una transferencia fue
  // cancelada o nunca existió, su IVA/comisión no puede seguir liquidado.
  // Se revierte una sola vez y se deja trazabilidad explícita.
  for (const transaction of [...nextTransactions]) {
    if (!transaction?.originalTransactionId || transaction.status !== "Settled") continue;
    const original = currentTransactions.get(transaction.originalTransactionId)
      || nextTransactions.find((item) => item.id === transaction.originalTransactionId);
    if (original?.status === "Settled") continue;
    const reverted = reverseDependentTransaction(nextAccounts, transaction, "orphaned_original_transaction");
    if (reverted) {
      const idx = nextTransactions.findIndex((item) => item.id === transaction.id);
      if (idx >= 0) nextTransactions.splice(idx, 1, {
        ...transaction,
        status: "Cancelled",
        cancellationReason: "orphaned_original_transaction",
        cancelledAt: updatedAt
      });
      auditLogs.push(auditLog("reverted_orphaned_transaction", transaction.id, "orphaned_original_transaction"));
    }
  }
  ...
  return {
    ...
    subsidyRequests: mergeById(current.subsidyRequests || [], incoming.subsidyRequests || []),
    investmentHoldings: mergeById(current.investmentHoldings || [], incoming.investmentHoldings || []),
...
export function applyServerTransaction(accounts, currentTransactions, currentOperations, paymentLinks, transaction, config = {}) {
  ...
  if (transaction.originalTransactionId) {
    ...
    if (!original || original.status !== "Settled") {
      return denied("original_transaction_not_settled");
    }
  }

  const isPending = transaction.status && transaction.status !== "Settled";
  if (isPending) {
    // Validaciones estructurales también aplican a Pending: no se guarda una
    // operación hacia una cuenta inexistente ni una inversión sin operación.
    if (!to && ![TGLP_ID, AGLDP_ID].includes(transaction.toAccountId)) {
      return denied(`target_not_found:${transaction.toAccountId}`);
    }
    if (transaction.kind === "InvestmentBuy" && to) {
    ...
    if (transaction.kind === "InvestmentSell" && !findOpenInvestmentOperation(currentOperations, transaction)) {
      return denied("investment_operation_not_open");
    }
    // Pending no reserva fondos: solo puede conservarse mientras la cuenta
    // pueda cubrir el débito completo. Nunca se crea un Pending insolvente.
    const totalDebit = debitFor(transaction.kind, amount, tax);
    if (from.id !== VAULT_EMISION && from.balancePz < totalDebit) {
      return denied("source_insufficient_balance");
    }
    return { ok: true };
  }
...
function reverseDependentTransaction(accounts, transaction, reason) {
  const from = accounts.get(transaction.fromAccountId);
  const to = accounts.get(transaction.toAccountId);
  const amount = safeAmount(transaction.amountPz);
  if (!from || amount <= 0) return false;
  const debit = debitFor(transaction.kind, amount, safeAmount(transaction.ivaPz ?? transaction.taxAmount));
  if (from.id !== VAULT_EMISION) from.balancePz = safeAmount(from.balancePz) + debit;
  if (to) to.balancePz = safeAmount(to.balancePz) - amount;
  return true;
}

function cancelledTransaction(previous, incoming, reason, updatedAt) {
  return {
    ...previous,
    ...incoming,
    status: "Cancelled",
    cancellationReason: reason,
    cancelledAt: incoming.cancelledAt || updatedAt || new Date().toISOString()
  };
}

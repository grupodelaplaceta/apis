...
export async function writeBankState(state) {
  ...
  try {
    ...
    const guarded = reconcileIncomingState(current, state, updatedAt);
...
function reconcileIncomingState(current, incoming, updatedAt) {
  ...
  for (const transaction of newTransactions) {
    const result = applyServerTransaction(nextAccounts, currentTransactions, currentOperations, transaction);
    if (!result.ok) {
  ...
  return {
    ...
    auditLogs,
    treasuryConfig: incoming.treasuryConfig || current.treasuryConfig || {},
    updatedAt
  };
}

function applyServerTransaction(accounts, currentTransactions, currentOperations, transaction) {

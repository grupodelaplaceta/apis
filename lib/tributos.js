...
import { ivaPorcentajeSync } from "./valores-bop.js";

// IVA normativo (CNI-BANCO Art. 4). Se resuelve en vivo desde el CNIC-IVA del
// BOLP con fallback 12 %; se mantiene VAT_RATE por compatibilidad interna.
const VAT_RATE = () => ivaPorcentajeSync() / 100;
...
function calculateInvoiceTotals(lines = []) {
  const normalizedLines = (lines || []).map((line) => {
    ...
    const ivaPercent = Number(line?.iva_porcentaje ?? ivaPorcentajeSync());
...
export async function createDeclarationForContributor(contributor) {
  ...
  const declaration = {
    ...
    dip: contributor.dip || normalizeDip(""),
...
/** Lista las declaraciones del contribuyente (más recientes primero). */
export async function listDeclarationsForContributor({ placetaId, dip }) {
  const collection = await declarationsCollection();
  const filter = placetaId
    ? { placeta_id: normalizePlacetaId(placetaId) }
    : dip
      ? { dip: normalizeDip(dip) }
      : {};
  const docs = await collection.find(filter).sort({ created_at: -1 }).limit(60).toArray();
  return docs.map((declaration) => ({
    id: declaration.id,
    mes_periodo: declaration.mes_periodo,
    placeta_id: declaration.placeta_id,
    cuenta_id_blp: declaration.cuenta_id_blp,
    patrimonio_medio: Number(declaration.patrimonio_medio || 0),
    indice_acumulacion: Number(declaration.indice_acumulacion || 0),
    cuota_irm: Number(declaration.cuota_irm || 0),
    cuota_igf: Number(declaration.cuota_igf || 0),
    exencion_aplicada: Boolean(declaration.exencion_aplicada),
    dias_activos_mes: Number(declaration.dias_activos_mes || 0),
    estado_pago: declaration.estado_pago || "Borrador",
    is_rectified: Boolean(declaration.is_rectified),
    created_at: declaration.created_at,
    updated_at: declaration.updated_at || declaration.created_at
  }));
}

 ...
 */
export function validarTransferenciaJunior(state, from, to, cantidad, mes) {
  ...
  const acumulado = movimientosMensualesJunior(state, junior.id, mes);
  const solicitado = Number(cantidad) || 0;
  if (acumulado + solicitado > LIMITE_MENSUAL_JUNIOR) {
    return {
      ok: false,
      error: 'limite_mensual_junior_excedido',
      limite: LIMITE_MENSUAL_JUNIOR,
      acumulado,
      solicitado
    };
  }

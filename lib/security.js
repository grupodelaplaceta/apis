...
export async function assertPlacetaIdBearer(req, res) {
...
function normalizePlacetaPayload(payload) {
  ...
  return {
    ...
    // Identidad adicional declarada por PlacetaID (el JWT de sesión la
    // incluye). Se usa para el alta bancaria: nombre visible y edad.
    nombre: String(payload?.nombreCompleto || payload?.nombre || "").trim(),
...
async function rememberNonce(appId, nonce) {
  try {
    ...
    await collection.insertOne(stripEmptyMongoKeys({
      appId,
      nonce,
      createdAt: new Date(),

...
export async function sendTransaction({ fromAccountId, toAccountId, amountPz, concept, kind = 'Transfer', ivaPz = 0, note = '', juniorDip = '', tutorDip = '' }) {
  ...
  // 7. Enviar estado actualizado al bridge y leer el saldo confirmado.
  await putBankState(state);
  const confirmed = esDemo ? state : await getBankState();
 ...
 */
export async function crearCuentaInfantil({ juniorDip, juniorNombre, tutorAccountId, tutorDip = '', sendLimitPz = 50 }) {

...
export async function sendTransaction({ fromAccountId, toAccountId, amountPz, concept, kind = 'Transfer', ivaPz = 0, note = '', juniorDip = '', tutorDip = '' }) {
  ...
  const confirmedTransaction = (confirmed.transactions || []).find(t => t.id === transaction.id);
  // El write/reconcile es la fuente de verdad. Nunca devolver éxito ni IVA
  // pagado si la operación principal fue cancelada por saldo, límites o una
  // validación posterior; así no se generan pagos fantasma.
  if (!esDemo && confirmedTransaction?.status !== 'Settled') {
    const reason = confirmedTransaction?.cancellationReason || 'transaction_not_settled';
    throw new Error(`Transacción no liquidada: ${reason}`);
  }
  ...
  const confirmedIva = !esDemo && ivaPz > 0
    ? (confirmed.transactions || []).find(t => t.originalTransactionId === transaction.id && t.status === 'Settled')
    : null;
  ...
  return {
    ...
    ivaPagado: confirmedIva ? ivaPz : 0,
    ivaTransactionId: confirmedIva?.id || null,
    esDemo
  };
}
